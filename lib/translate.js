...
function buildLlmPrompt(texts, target, domain) {
  ...
  return (
    ...
    `Dịch từng đoạn được đánh số dưới đây sang ${lang}. ` +
...
async function anthropicBatch(texts, target, apiKey, model, domain) {
  const r = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 8000,
      messages: [{ role: "user", content: buildLlmPrompt(texts, target, domain) }],
    }),
  });
...
async function openaiBatch(texts, target, apiKey, model, domain) {
...
export async function translateMany(rawTexts, opts = {}) {
  ...
  const bucket = (isLLM ? "llm" : "mt") + ":" + domain;
  ...
  // Khoá thuật ngữ chuyên ngành cho bộ dịch máy (LLM tự lo qua câu lệnh).
  const gloss = isLLM ? null : glossaryFor(domain);
  ...
  async function run(name) {
    if (name === "google") return googleBatch(sendTexts, target);
    if (name === "mymemory") return myMemoryBatch(sendTexts, target);
    if (name === "anthropic")
      return anthropicBatch(missTexts, target, opts.apiKey, opts.model, domain);
    if (name === "openai")
      return openaiBatch(missTexts, target, opts.apiKey, opts.model, domain);
    if (name === "deepl") return deeplBatch(sendTexts, target, opts.apiKey);
